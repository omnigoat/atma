...
{
	template <typename T>
	concept memory_concept = requires(T x)
	{
		typename T::value_type;
		typename T::allocator_type;
		{ std::data(x) };
		{ atma::get_allocator(x) };
	};

	template <typename T>
	concept bounded_memory_concept = requires(T x)
	{
		{ std::size(x) };

		requires memory_concept<T>;
	};

	template <typename T>
	concept dest_memory_concept = requires(T x)
	{
		memory_concept<T>;

		std::is_same_v<typename T::tag_type, dest_memory_tag_t>;
	};

	template <typename T>
	concept src_memory_concept = requires(T x)
	{
		memory_concept<T>;

		std::is_same_v<src_memory_tag_t, typename T::tag_type>;
	};

	template <typename T>
	concept dest_bounded_memory_concept = requires(T x)
	{
		requires bounded_memory_concept<T>;
		requires std::same_as<typename rmref_t<T>::tag_type, dest_memory_tag_t>;
	};
	...
	template <typename T>
	concept sized_range_concept = requires(T&& x)
	{
		models_v<range_concept, rmref_t<T>>;
...
namespace atma::detail
{
	...
	struct xfer_make_from_sized_contiguous_range_
	{
		...
		requires sized_range_concept<rmref_t<R>>
	...
	{
		template <typename M>
		requires memory_concept<rmref_t<M>>
...
namespace atma
{
	...
	requires memory_concept<rmref_t<decltype(dest)>>
	{
...
namespace atma
{
	constexpr auto memory_construct = functor_list_t
	{
		...
		[](auto&& dest, auto&&... args)
		requires sized_range_concept<decltype(dest)>
		{
...
namespace atma::detail
{
	...
	constexpr auto _memory_copymove_ = functor_list_t
	{
		...
		[]<typename F, dest_memory_concept D, src_memory_concept S>
		(F& f, D&& dest, S&& src, size_t sz)
		{
		...
		[]<typename F, dest_bounded_memory_concept D, src_bounded_memory_concept S>
		(F& f, D&& dest, S&& src)
		{
		...
		[]<typename F, dest_bounded_memory_concept D, src_memory_concept S>
		(F& f, D&& dest, S&& src)
		{
			f(get_allocator(dest),
				std::data(dest),
		...
		[]<typename F, dest_memory_concept D, src_bounded_memory_concept S>
		(F& f, D&& dest, S&& src)
		{
			f(get_allocator(dest),
				std::data(dest),
		...
		[]<typename F, dest_memory_concept D, std::ranges::contiguous_range S>
		(F& f, D&& dest, S&& src)
		{
		...
		[]<typename F, sized_range_concept D, sized_range_concept S>
		(F& f, D&& dest, S&& src)
		{
			using allocator_type = decltype(detail::allocator_type_of_range_(dest));
		...
		[]<typename F, dest_memory_concept D, std::forward_iterator IterB, typename IterE>
		(F& f, D&& dest, IterB begin, IterE end)
		{
			f(get_allocator(dest),
				std::data(dest),
				begin, end);
...
namespace atma
{
	constexpr auto memory_relocate_range = functor_list_t
	{
		...
		requires
			dest_memory_concept<rmref_t<decltype(dest)>> &&
			src_memory_concept<rmref_t<decltype(src)>>
		{
			constexpr bool dest_is_bounded = bounded_memory_concept<decltype(dest)>;
			constexpr bool src_is_bounded =  bounded_memory_concept<decltype(src)>;
		...
		[]<dest_bounded_memory_concept D, src_bounded_memory_concept S>
		(D&& dest, S&& src)
		{
			ATMA_ASSERT(std::size(dest) == std::size(src));
		...
		[](auto&& dest, auto&& src)
		requires
			dest_bounded_memory_concept<decltype(dest)>&&
			src_memory_concept<decltype(src)>
		{
			detail::_memory_relocate_range(
				get_allocator(dest),
				get_allocator(src),
		...
		[](auto&& dest, auto&& src)
		requires
			dest_memory_concept<decltype(dest)>&&
			src_bounded_memory_concept<decltype(src)>
		{
			detail::_memory_relocate_range(
				get_allocator(dest),
				get_allocator(src),
